...
class Server:
    def __init__(self, batch_size: int, n_pages: int, page_size: int, n_heads: int, head_dim: int):
        ...
        self.model = torch.compile(PagedAttentionLayer(n_heads, head_dim, torch.bfloat16, self.paged_attention))
        ...
        self.request_length = torch.tensor([float('inf')] * batch_size, device="cuda") # decide whether a request is completed

        self.token_embedding = torch.randn((batch_size, 1, n_heads*head_dim), device="cuda", dtype=torch.bfloat16) # [B, 1, n_heads*head_dim]

        self.block_mask = create_block_mask(lambda b,h,q,kv: q >= kv, batch_size, 1, 64*1024, 64*1024, BLOCK_SIZE=page_size)
    ...
    def can_schedule(self, request: Tuple[int, int]) -> bool:
        return len(self.paged_attention.empty_pages) * self.paged_attention.page_size >= sum(request)
    ...
    def prefill_one_token(self, batch_idx: int, prompt_len: int, response_len: int):
        ...
        self.paged_attention.reserve(torch.tensor(batch_idx, device="cuda"), torch.tensor(prompt_len+response_len, device="cuda"))
        ...
        converted_block_mask = self.paged_attention.convert_logical_block_mask(new_block_mask, torch.tensor([batch_idx], device="cuda"))
        ...
        token_embedding = self.model(torch.tensor([batch_idx], device="cuda"), prefill_input_pos, token_embedding, converted_block_mask, converted_score_mod)
        return token_embedding

    def prefill(self):
        while self.request_queue and self.can_schedule(self.request_queue[0]) and self.available_batch_idx:
            prompt_len, response_len = self.request_queue.popleft()
            print(f"serving a new request with prompt_len: {prompt_len}, response_len: {response_len}")
    ...
    def get_decode_mask(self, batch_idx: torch.Tensor, input_pos: torch.Tensor):
        ...
        if self.block_mask.full_kv_num_blocks is not None:
            full_kv_num_blocks = self.block_mask.full_kv_num_blocks[batch_idx, :, input_block_idx].view(B, 1, 1)
            full_kv_indices = self.block_mask.full_kv_indices[batch_idx, :, input_block_idx].view(B, 1, 1, -1)
        seq_length = (1, self.block_mask.seq_lengths[1])
        return BlockMask.from_kv_blocks(kv_num_blocks, kv_indices, full_kv_num_blocks, full_kv_indices, BLOCK_SIZE=self.block_mask.BLOCK_SIZE, mask_mod=gen_offset(input_pos), seq_lengths=seq_length)
    ...
    def decode(self):
        ...
        converted_score_mod = self.paged_attention.get_score_mod(_identity)
        self.token_embedding[batch_idx] = self.model(batch_idx, input_pos.view(B, 1), self.token_embedding[batch_idx], converted_block_mask, converted_score_mod)
    ...
    def clean(self):
        ...
        self.batch_idx = [idx for idx in self.batch_idx if idx not in completed_batch_indices.tolist()]
